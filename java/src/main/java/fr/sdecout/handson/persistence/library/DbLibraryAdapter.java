...
class DbLibraryAdapter implements LibraryAccess, LibrarySearch, LibraryCreation, BookCollectionUpdate {
    ...
    public Optional<LibraryResponse> findLibrary(LibraryId id) {
        return dsl.selectFrom(LIBRARY)
                ...
                .fetchOptional(row -> new LibraryResponse(
                        row.getId(),
                        row.getName(),
                        new AddressField(row.getAddressLine_1(), row.getAddressLine_2(), row.getPostalCode(), row.getCity()))
                );
    }
    ...
    public Stream<LibrarySearchResponseItem> searchLibrariesClosestTo(PostalCode postalCode) {
        return dsl.selectFrom(LIBRARY)
                ...
                .fetch(row -> new LibrarySearchResponseItem(
                        row.getId(),
                        row.getName(),
                        new AddressField(row.getAddressLine_1(), row.getAddressLine_2(), row.getPostalCode(), row.getCity()))
                ).stream();
    ...
    public Stream<LibrarySearchResponseItem> searchLibrariesWithBookAvailable(Isbn isbn) {
        return dsl.select()
                ...
                .fetch(row -> new LibrarySearchResponseItem(
                        row.get(LIBRARY.ID),
                        row.get(LIBRARY.NAME),
                        new AddressField(
                                row.get(LIBRARY.ADDRESS_LINE_1),
                                row.get(LIBRARY.ADDRESS_LINE_2),
                                row.get(LIBRARY.POSTAL_CODE),
                                row.get(LIBRARY.CITY)
                        )
                )).stream();
    }
