...
class DbBookAdapter implements BookAccess, BookSearch, BookUpdate {
    ...
    public Stream<BookSearchResponseItem> searchBooks(String hint) {
        return dsl.select(BOOK.ISBN, BOOK.TITLE, bookAuthors)
                ...
                .fetch(it -> new BookSearchResponseItem(new BookField(
                        new Isbn(it.get(BOOK.ISBN)).formattedValue(),
                        it.get(BOOK.TITLE),
                        it.get(bookAuthors)
                ))).stream();
    }
    ...
    public void save(Isbn isbn, String title, Collection<AuthorId> authors) {
        ...
        dsl.batch(concat(Stream.of(upsertOperation, removeAuthorOperation), addAuthorOperations).toList()).execute();
    }
    ...
    private static final Field<List<AuthorField>> bookAuthors = multiset(
